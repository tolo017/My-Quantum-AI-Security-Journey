...
Instead of starting containers one by one, we use a single file to define our entire world. I've created a `docker-compose.yml` in the `src/` folder.
...
We don't want a "fork bomb" or heavy script to freeze your host. We will modify the `docker-compose.yml` to limit each container to **2GB of RAM** and **1.5 CPU**.
...
3. Try to `ping` one from the other. It should fail/accept(according to your setup). This is **Segmentation**—blocking lateral movement.
